...
const tabValue = computed<string>({ get: () => (recurrent.value.value ? "recurrente" : "fijo"), set: v => (recurrent.value.value = v === "recurrente") })
...
function toIso(dateVal: unknown, time: string, duration: number) {
  ...
  const [h, m] = time.split(":").map(Number); const start = new Date(base); start.setHours(h, m, 0, 0); const end = new Date(start); end.setHours(end.getHours() + duration)
  return { startISO: start.toISOString(), endISO: end.toISOString(), endHuman: end.toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" }) }
}
...
function syncRow(i: number) {
  ...
  appts.value.value[i] = { ...(appts.value.value[i] || {}), start_date: startISO, end_date: endISO, status: appts.value.value[i]?.status ?? "pending", payment_status: appts.value.value[i]?.payment_status ?? "unpaid", extra_hours: appts.value.value[i]?.extra_hours ?? 0, total_cost: appts.value.value[i]?.total_cost ?? 0 }
  if (!recurrent.value.value) { appts.value.value = [appts.value.value[0]]; rows.value = [rows.value[0]] }
}

const canAdd = computed(() => recurrent.value.value && rows.value.length < maxCitas && isComplete(rows.value[rows.value.length - 1]))
function addRow() {
  if (!canAdd.value) return
  const last = rows.value[rows.value.length - 1]; let nextDateVal: unknown | null = null
  if (last.dateVal) { const base = toJsDate(last.dateVal); if (base) { const d = new Date(base); d.setDate(base.getDate() + 1); nextDateVal = fromDate(d, tz) } }
...
watch(() => recurrent.value.value, isRec => {
  if (!isRec) { rows.value = [rows.value[0] ?? { dateVal: null, time: "08:00", duration: 1 }]; appts.value.value = appts.value.value?.length ? [appts.value.value[0]] : [] }
  else if (rows.value.length === 0) rows.value.push({ dateVal: null, time: "08:00", duration: 1 })
})
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div class="space-y-3">
      <div class="flex items-center gap-2 max-w-3xl mx-auto">
        <TooltipProvider><Tooltip>
          ...
          <TooltipContent class="max-w-xs text-xs">Un servicio fijo ocurre en una sola fecha y hora; un servicio recurrente ocurre en varias fechas.</TooltipContent>
        </Tooltip></TooltipProvider>
        <Label class="text-sm font-medium">Tipo</Label>
      </div>
      <Tabs v-model="tabValue"><TabsList class="grid w-full grid-cols-2 max-w-3xl mx-auto"><TabsTrigger value="fijo">Fijo</TabsTrigger><TabsTrigger value="recurrente">Recurrente</TabsTrigger></TabsList></Tabs>
    </div>
    ...
    <div class="space-y-3">
            ...
            <div class="p-1 w-full">
                ...
                <CardContent class="p-0 w-full">
                    ...
                    <div class="w-full">
                        ...
                        <SelectTrigger class="h-9 w-full"> <SelectValue placeholder="Duración" class="font-medium"/></SelectTrigger>
                        <SelectContent>
                            <SelectGroup>
                    ...
                    <div class="w-full relative">
                        <div class="text-[11px] text-muted-foreground mb-1">Termina</div>
                        <Input
                    ...
                    <Button v-if="recurrent.value.value && rows.length>1" variant="outline" size="sm" class="h-8" @click="removeRow(i)">
                        Eliminar
                    </Button>
                    </div>
                </CardContent>
      ...
      <p v-if="appts.errorMessage" class="text-xs text-red-500">{{ appts.errorMessage }}</p>
    </div>
  </div>
</template>
